...
from scapy.layers.inet import ICMP, IP, Ether, TCP
...
def arp_scan(target):
    ...
    answered, unanswered = srp(arp_packet, timeout=2, retry=2, verbose=0)
    ...
    for answer in answered:
        results[answer[1].psrc] = {
            ...
            # convert to ms
            "latency": (answer[0][0].sent_time - answer[0][1].time) * 1000,
            "ports": {},
        }
...
def print_save_report(results):
    ...
    for host in hosts:
        ...
        if results[host]["latency"] is not None:
            print("Host is up ({}ms latency).".format(
                results[host]["latency"]))
            print("Host is up ({}ms latency).".format(
                results[host]["latency"]), file=savefile)

        if len_open == 0 and len_closed == 0:
            print("All {} scanned ports on {} are most likely filtered".format(
                len(ports), host))
            print(
            ...
            print("All {} scanned ports on {} are closed".format(
                len(ports), host), file=savefile)
        else:
            print("Not shown: {} filtered ports and {} closed ports".format(
                len_filtered, len_closed))
            print("Not shown: {} filtered ports and {} closed ports".format(
                len_filtered, len_closed), file=savefile)
        if len_open != 0:
            print("{:<12}{:<8}{:<20}".format("PORT", "STATE", "SERVICE"))
            print("{:<12}{:<8}{:<20}".format(
                "PORT", "STATE", "SERVICE"), file=savefile)
            for port in filter(lambda s: s[1] == "open", ports):
                try:
                    print("{:<12}{:<8}{:<20}".format(
                        str(port[0]) + "/tcp", "open", socket.getservbyport(int(port[0]))))
                    print("{:<12}{:<8}{:<20}".format(
                        str(port[0]) + "/tcp", "open", socket.getservbyport(int(port[0]))), file=savefile)
                except socket.error:
                    print("{:<12}{:<8}{:<20}".format(
                        str(port[0]) + "/tcp", "open", "unknown"))
                    print("{:<12}{:<8}{:<20}".format(
                        str(port[0]) + "/tcp", "open", "unknown"), file=savefile)

        if results[host]["MAC Address"] is not None:
            print("MAC Address: {}".format(results[host]["MAC Address"]))
            print("MAC Address: {}".format(
                results[host]["MAC Address"]), file=savefile)
            print("")
            print("", file=savefile)
...
if __name__ == "__main__":
    ...
    print("TCP Scan completed in {} seconds".format(
        scan_completed_time - start_scan_time))
    print("TCP Scan completed in {} seconds".format(
        scan_completed_time - start_scan_time), file=savefile)
